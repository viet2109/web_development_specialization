...
public class FileService {
    ...
    public FileResponseDto upload(MultipartFile file) {
        ...
        try {
            ...
            File fileEntity = File.builder().size(file.getSize()).fileCloudId(result.get("public_id").toString()).name(file.getOriginalFilename()).type(mimeType).path(result.get("secure_url").toString()).build();
